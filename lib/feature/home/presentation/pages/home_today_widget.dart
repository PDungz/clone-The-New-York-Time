...
import 'package:news_app/feature/home/presentation/web/home_webview_page.dart';
...
class HomeTodayWidget extends StatelessWidget {
  const HomeTodayWidget({super.key});
    ...
    return GestureDetector(
      onTap:
          () => Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => HomeWebviewPage(url: article.url ?? ''),
            ),
          ),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            if (article.publishedDate != null &&
                FormatDate.isDurationWithin(
                  DateTime.now(),
                  DateTime.parse(article.publishedDate!),
                  Duration(hours: 6),
                ))
              Text(
                FormatDate.formatTimeAgoShort(
                  DateTime.parse(article.publishedDate!),
                ),
            ...
            if (hasImage) ...[
              ...
              CachedImageWidget(
                ...
                localImagePath: article.multimedia!.first.localPath ?? '',
